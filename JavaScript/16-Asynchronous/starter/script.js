...
const request = fetch('https://restcountries.com/v3/name/potugal');
console.log(request);
...
const whereAmI = async function () {
  try {
    ...
    //Reverse geocoding
    const locCountry = await fetch(
      `https://geocode.xyz/${lat},${lng}?geoit=json`
    );
    if (!locCountry.ok) throw new Error('Problem getting location data');
    const data1 = await locCountry.json();

    // Country Data
    // const resp = await fetch(`https://restcountries.com/v3/name/${country}`).then(response => console.log(response));

    const resp = await fetch(
      `https://restcountries.com/v3/name/${data1.country}`
    );
    if (!resp.ok) throw new Error('Problem getting location data');
    const data = await resp.json();
    renderCountry(data[0]);

    return `You are in ${data1.city}, ${data1.state}`;
  } catch (err) {
    console.error(err);
    renderError(`Something went wrong😔 ${err.message}`);

    // Reject promise thrown from Async error
    throw err;
  }
};
...
(async function () {
  try {
    ...
    if (!location.ok) throw new Error('The location could not be found');
...
const threeCountries = async function (c1, c2, c3) {
  try {
...
threeCountries('portugal', 'canada', 'Tanzania');
//////////////////////////////////////////////////////////////////////////////////////////////////////
...
const timeout = function (sec) {
  return new Promise(function (_, reject) {
    setTimeout(function () {
      reject(new Error('Request denied: took too long'));
    }, sec);
  });
};

Promise.race([getJSON(`https://restcountries.com/v3/name/italy`), timeout(200)])
